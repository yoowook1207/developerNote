...
> - **Promise** is immediately called, but it can not stop; **Observable** needs to subscribe, and it can be stopped by unsubscribe function.
> - **Promise** push errors to its child promises; **Observable** will handle the error in subscribe method.
> - **Promise** only to use .then() .catch() .finally() clause to manipulate the data step by step;**Observable** uses RxJs operators.
> - **Observable** base on Rxjs library, it provides a lot of methods to manipulate the data by using pipe() and subscription to handle complex applications.